...
function membersSlider() {
   ...
   function showSlide(index) {
      ...
      nextBtn.style.backgroundColor = index === totalSlides ? 'rgba(49, 49, 49, 0.2)' : '';
      nextBtn.disabled = index === totalSlides;

      prevBtn.style.backgroundColor = index === 1 ? 'rgba(49, 49, 49, 0.2)' : '';
...
function stagesSlider() {
   ...
   const prevBtn = stagesWrapper.querySelector('.stages__arrows--left');
   const nextBtn = stagesWrapper.querySelector('.stages__arrows--right');
   const dotsContainer = stagesWrapper.querySelector('.stages__dots');

   const updateArrows = () => {

      if (currentSlide === 0) {
         prevBtn.style.backgroundColor = 'rgba(49, 49, 49, 0.2)';
      ...
      if (currentSlide === stages.length - 1) {
         nextBtn.style.backgroundColor = 'rgba(49, 49, 49, 0.2)';
   ...
   const updateSlidePosition = () => {
      const padding = 20;
      const slideWidth = stagesWrapper.offsetWidth + padding;
   ...
   prevBtn.addEventListener('click', () => {
      currentSlide = currentSlide === 0 ? stages.length - 1 : currentSlide - 1;
      updateSlidePosition();
      updateArrows();
   });

   nextBtn.addEventListener('click', () => {
      currentSlide = currentSlide === stages.length - 1 ? 0 : currentSlide + 1;
      updateSlidePosition();
      updateArrows();
   });
